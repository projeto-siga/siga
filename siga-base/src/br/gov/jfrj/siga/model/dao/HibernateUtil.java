...
public class HibernateUtil {
	...
	public static void configurarHibernate(String resource) {
		configurarHibernate(resource, null);
	}
	...
	public static void configurarHibernate(String resource,
		...
		try {
			...
			if (fUseDatasource != null) {
				if (fUseDatasource) {
					conf.setProperty("connection.datasource", conf
							.getProperty("siga.connection.datasource"));
					...
					conf.setProperty("hibernate.connection.url", conf
							.getProperty("siga.hibernate.connection.url"));
					...
					conf
							.setProperty(
									"hibernate.connection.driver_class",
									conf
											.getProperty("siga.hibernate.connection.driver_class"));
				}
			}
			...
			if (hibernateConnectionUrl != null
				...
				conf.getProperties().remove("connection.datasource");
				conf.setProperty("hibernate.connection.url", conf
						.getProperty("siga.test.hibernate.connection.url"));
				conf
						.setProperty(
				...
				conf
						.setProperty(
								"hibernate.connection.driver_class",
								conf
										.getProperty("siga.test.hibernate.connection.driver_class"));
			}
			...
			ex.printStackTrace();
			HibernateUtil.logger.error(
					"Não foi possível configurar o hibernate.", ex);
			throw new ExceptionInInitializerError(ex);
		}
	}

	public static void configurarHibernate(AnnotationConfiguration cfg,
			String hibernateConnectionUrl, Class... classesAnotadas) {
		try {
