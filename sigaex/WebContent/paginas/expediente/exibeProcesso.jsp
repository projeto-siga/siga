...
<siga:links>
	...
	<input type="radio" id="radioPDFSemMarcas" name="formato" value="pdfsemmarcas"
	...
	<tr>
		...
		<table class="list" width="100%">
			...
			<c:forEach var="arqNumerado" items="${arqsNum}">
				<tr>
					<td style="padding-left: ${arqNumerado.nivel * 6}pt">
						<c:if test="${!empty arqNumerado.arquivo.resumo}">
   						...
   						<a title="${fn:substring(tooltipResumo,0,fn:length(tooltipResumo)-4)}" href="javascript:exibir('${arqNumerado.referenciaHtml}','${arqNumerado.referenciaPDF}','')">${arqNumerado.nome}</a>
			...
			<c:if test="${!empty possuiResumo}">
				<tr>
				<ww:url id="url" action="exibirResumoProcesso" namespace="/expediente/doc">
					<ww:param name="sigla">${mob.sigla}</ww:param>
				</ww:url>
					<td colspan="3" style="background-color: silver">
					 <a href="javascript:exibirNoIFrame('${url}')">RESUMO</a>
					</td>
				</tr>
			</c:if>
		...
		<td valign="top" id="paipainel"
			...
			width="75%"><iframe
			style="visibility: visible; margin: 0px; padding: 0px;" name="painel"
			id="painel" src="" align="right" width="100%" onload="resize();"
...
function exibir(refHTML,refPDF,semMarcas) {
	...
	if (document.getElementById('radioHTML').checked && refHTML != ''){
		...
		if(navigator.appName!='Microsoft Internet Explorer'){
			...
			ifr.style.height = pageHeight() - 150;
		}
