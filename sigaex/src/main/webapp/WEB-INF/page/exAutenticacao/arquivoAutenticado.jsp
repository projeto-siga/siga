...
<siga:pagina titulo="Movimentação" desabilitarmenu="sim" onLoad="try{var num = document.getElementById('id_number');if (num.value == ''){num.focus();num.select();}else{var cap = document.getElementById('id_captcha');cap.focus();cap.select();}}catch(e){};">
	<div class="gt-bd clearfix">
		<div class="gt-content clearfix">
			...
			<div>
				<c:url	var='pdfAssinado' value='/app/configuracao/arquivoAutenticado_stream?n=${n}&answer=${answer}&assinado=true' /> 
				<c:url	var='pdf' value='/app/configuracao/arquivoAutenticado_stream?n=${n}&answer=${answer}&assinado=false' />
			...
			<div>
				...
				<ul>
					<c:forEach var="assinatura" items="${assinaturas}" varStatus="loop">
						<c:url	var='arqAssinatura' value='/app/configuracao/arquivoAutenticado_stream?n=${n}&answer=${answer}&idMov=${assinatura.idMov}' /> 
						<li><a href="${arqAssinatura}" target="_blank">${assinatura.descrMov}</a></li>
					</c:forEach>
				</ul>
				<br/>
				<a href="${request.contextPath}/app/configuracao/autenticar" class="gt-btn-large gt-btn-left">Autenticar outro Documento</a>
				<c:if test="${mostrarBotaoAssinarExterno}">
				<div style="padding-left: 10;">
					<div id="dados-assinatura" style="visible: hidden">
						<input type="hidden" name="pdfchk_${mov.idMov}" value="${mov.referencia}" />
						<input type="hidden" name="urlchk_${mov.idMov}" value="/app/configuracao/arquivoAutenticado_stream?n=${n}&answer=${answer}&assinado=false" />

						<c:set var="jspServer" value="${request.contextPath}/app/configuracao/autenticar?n=${n}&ass=1&answer=${answer}" />
						<c:set var="nextURL" value="${request.contextPath}/app/configuracao/autenticar" />
