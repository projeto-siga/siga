...
/*		operacoesDocumentoPage.clicarLinkDesfazerRedefinicaoSigilo();
		Assert.assertNotNull(util.getWebElement(driver, By.xpath("(//p/b[contains(.,'P�blico')])")), "N�vel de acesso não foi modificado para p�blico");*/
	}
	...
	public void definirPerfil() throws InterruptedException {
		...
		Assert.assertTrue(divPerfil.getText().toUpperCase().contains(propDocumentos.getProperty("nomeResponsavel").toUpperCase()), "Nome do usuário respons�vel não encontrado!");
	...
	public void solicitarPublicacaoBoletim() {
		...
		if(Calendar.getInstance().get(Calendar.HOUR_OF_DAY) >= 17) {
			Assert.assertNotNull(util.getWebElement(driver, By.xpath("//h3[contains(., 'A solicitação de publicação no BIE apenas � permitida at� as 17:00')]")),
					"Texto 'A solicitação de publicação no BIE apenas é permitida até as 17:00' não foi encontrado!");
		} else {
			Assert.assertNotNull(util.getWebElement(driver, By.xpath("//td[3][contains(., 'Solicita��o de Publica��o no Boletim')]")), "Texto 'Solicita��o de Publica��o no Boletim' não foi encontrado!");
			...
			Assert.assertNotNull(util.getWebElement(driver, By.linkText("Solicitar Publicação no Boletim")), "Texto Solicitar Publica��o no Boletim não foi encontrado!");
		}
	}
	...
	public void vincularDocumento() {
		...
		Assert.assertTrue(util.isElementInvisible(driver, By.id("outputRelacaoDocs")), "�rea de Documentos Relacionados ainda está visível!");
	}
	...
	public void apensarDocumento() {
		...
		//Assert.assertNotNull(documentosRelacionados, "�rea de Documentos Relacionados não foi encontrada!");
	...
	public void transferirDocumento() {
		...
		Assert.assertNotNull(util.getWebElement(driver, By.xpath(OperacoesDocumentoPage.XPATH_STATUS_DOCUMENTO + "[contains(text(), 'A Receber (F�sico)')]|//div[h3 = 'Vias']/ul/li[contains(., 'A Receber (F�sico)')]")), "Texto 'A Receber (F�sico)' não foi encontrado!");
