...
//	private List<WfTipoResponsavel> listaTipoResponsavel = new ArrayList<WfTipoResponsavel>();
	 ...
	 */
	@Inject
	public ConfiguracaoController(HttpServletRequest request, Result result, WfDao dao, SigaObjects so, WfUtil util) {
		super(request, result, dao, so, util);

//		WfTipoResponsavel tpIndefinido = new WfTipoResponsavel(TIPO_RESP_INDEFINIDO, "[Indefinido]", "");
//		WfTipoResponsavel tpMatricula = new WfTipoResponsavel(TIPO_RESP_MATRICULA, "Matrícula", "matricula");
