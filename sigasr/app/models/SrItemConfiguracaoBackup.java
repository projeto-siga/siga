...
public class SrItemConfiguracaoBackup extends GenericModel implements SrSelecionavel {
	...
	public SrItemConfiguracaoBackup() {

	}

	public SrItemConfiguracaoBackup(String sigla, String descricao) {
		super();
	...
	public SrSelecionavel selecionar(String sigla) {
		setSigla(sigla);
		List<SrItemConfiguracaoBackup> itens;
		if (this.siglaItemConfiguracao != null)
			itens = SrItemConfiguracaoBackup.find("bySiglaItemConfiguracao", this.siglaItemConfiguracao).fetch();
		else
			itens = SrItemConfiguracaoBackup.find("byDescrItemConfiguracaoLike",
	...
	public List<SrSelecionavel> buscar() {
		...
		return SrItemConfiguracaoBackup.find(query).fetch();
	}
	...
	public SrItemConfiguracaoBackup getPorNivelHierarquia(int nivel) {
		String codigo = "";
		if (nivel > getNivel())
			return new SrItemConfiguracaoBackup();
		String[] partes = siglaItemConfiguracao.split("\\.");
		String ponto = "";
		...
		return SrItemConfiguracaoBackup.find("bySiglaItemConfiguracao", codigo).first();
	}

	public List<SrItemConfiguracaoBackup> getOpcoesPorNivel(int i) {
		if (i > getNivel() + 1)
			return new ArrayList<SrItemConfiguracaoBackup>();
		String zeros = "";
		String codigo = "";
		int nZeros = NIVEIS - i;
		...
		for (int n = 0; n < i - 1; n++)
			codigo += partes[n] + ".";
		List<SrItemConfiguracaoBackup> lista = SrItemConfiguracaoBackup
