...
public class GeradorNTFS {
	...
	public static void main(String[] args) throws Exception {
		processarParamentros(args);
		AnnotationConfiguration cfg = (AnnotationConfiguration) CpDao.criarHibernateCfg(conf
...
//					&& !pastaNova.contains(("\\DIR-"))) {
			if (lot == null){
				throw new AplicacaoException("Problema na lotaï¿½ï¿½o (nï¿½o existe ou estï¿½ definida como lotaï¿½ï¿½o ADMINISTRATIVA e JUDICIAL ao mesmo tempo): " + nomePastaNova);
			}
...
//				criarPasta(pastaNova + "\\Juizes", perms);
	...
	private static void processarParamentros(String[] args) throws AplicacaoException {
		try{
			ambiente = args[0].substring(1);			
		}catch (Exception e) {
			throw new AplicacaoException("Defina o ambiente! (Ex: -desenv, -homolo ou -prod)");
		}
		
		try{
			orgao = args[1].substring(1);			
		}catch (Exception e) {
			throw new AplicacaoException("Defina o ï¿½rgï¿½o! (Ex: -jfrj, -trf2 ou -cjf)");
		}

		
		conf = SincProperties.getInstancia("siga.cp.sinc.ldap." +orgao + "." + ambiente);
		
		servidor = conf.getNtfsServidor();
		usuario = conf.getNtfsUsuario();
		senha = conf.getNtfsSenha();
		pathRaiz = conf.getNtfsPathRaiz();
	...
	private static void criarPasta(String path, List<Permissao> permissoes)
		...
		for (Permissao p : permissoes) {
			String cmdPermissao = "icacls " + path + " /grant " + p.getObjetoAD()
					+ ":" + p.getConcessao() + " /inheritance:d";
			comandosPermissao.add(cmdPermissao);
			
			String cmdRemoveCreatorOwner = "icacls " + path + " /remove:g \"creator owner\"";
			comandosPermissao.add(cmdRemoveCreatorOwner);
			
			String cmdRemoveUsers = "icacls " + path + " /remove:g \"users\"";
			comandosPermissao.add(cmdRemoveUsers);
			...
			if (!nomePasta.equals("Gabinete") && !nomePasta.equals("Secretaria") && !nomePasta.equals("Juizes") && !nomePasta.equals("Publica")){
				String concessaoCompartilhamento = p.getConcessao().equals("(RX,DC,GW)")?"CHANGE":"READ";
