	...
	<c:if test="${not empty url}">
		<c:choose>
			<c:when test="${not empty popup and popup != false}">
				<a class="${classe} ${linkBotoes ? 'link-btn btn btn-sm btn-light' : ''}" <c:if test="${not empty accesskey}">accesskey="${accesskey}"</c:if>
					href="javascript:${linkConfirm}popitup('${url}');">${img}${title}</a>
			</c:when>
			<c:when test="${not empty ajax and ajax != false}">
				<span id="spanAjax_${idAjax}"> <a class="${classe} link-btn btn btn-sm btn-light" <c:if test="${not empty accesskey}">accesskey="${accesskey}"</c:if>
			...
			<c:otherwise>
				<c:choose>
					<c:when test="${not empty linkConfirm}">
						<a class="${classe} link-btn btn btn-sm btn-light" <c:if test="${not empty accesskey}">accesskey="${accesskey}"</c:if>
							href="javascript:${linkConfirm}location.href='${url}';">${img}${title}</a>
					</c:when>
					<c:otherwise>
						<a class="${classe} link-btn btn btn-sm btn-light" <c:if test="${not empty accesskey}">accesskey="${accesskey}"</c:if> href="${url}">${img}${title}</a>
					</c:otherwise>
				</c:choose>
			</c:otherwise>
		</c:choose>
