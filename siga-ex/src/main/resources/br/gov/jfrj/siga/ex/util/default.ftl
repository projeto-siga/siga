...
[#macro field var index='' title=var+index kind="" width="" columns=80 lines=3 maxchars="" refresh=false required=false value="" default="" options="" searchClosed=false atts={} altered="" id="" ]
	[@campo var=var+index titulo=title tipo=kind largura=width colunas=columns linhas=lines maxcaracteres=maxchars reler=refresh obrigatorio=required valor=value default=default opcoes=options buscarFechadas=searchClosed atts=atts alterado=altered id=id /]
[/#macro]

[#macro value var index='' title=var+index kind="" width="" columns=80 lines=3 maxchars="" refresh=false required=false value="" default="" options="" searchClosed=false atts={} altered="" id="" ]${(.vars[var+index])!}[/#macro]
...
[#macro for expr depend='']
	...
	[#if depend??]
		[@grupo depende=depend]
			[#if max?? && max > 0]
				[#list 1..max as index][#nested index][/#list]
			[/#if]
		[/@grupo]
	[#else]
		[#if max?? && max > 0]
