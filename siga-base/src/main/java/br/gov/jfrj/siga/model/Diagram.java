...
public class Diagram {
			...
			for (int i = 0; i < f.length; i++) {
				...
				for (int j = 0; j < m.length; j++) {
					if (m[j].getName().equals(nameGetter)
							&& m[j].getReturnType().getName().equals(
									f[i].getType().getName())
	...
	public void createGraphML(String filename, boolean fIncludeAttributes,
		...
		writer
				.write("<graphml xmlns=\"http://graphml.graphdrawing.org/xmlns\" xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" xmlns:y=\"http://www.yworks.com/xml/graphml\" xmlns:yed=\"http://www.yworks.com/xml/yed/3\" xsi:schemaLocation=\"http://graphml.graphdrawing.org/xmlns http://www.yworks.com/xml/schema/graphml/1.1/ygraphml.xsd\">");
		writer.write("<!--Created by yFiles for Java 2.8-->");
		writer
				.write("<key for=\"graphml\" id=\"d0\" yfiles.type=\"resources\"/>");
		writer
				.write("<key for=\"port\" id=\"d1\" yfiles.type=\"portgraphics\"/>");
		writer
				.write("<key for=\"port\" id=\"d2\" yfiles.type=\"portgeometry\"/>");
		writer
				.write("<key for=\"port\" id=\"d3\" yfiles.type=\"portuserdata\"/>");
		writer
				.write("<key attr.name=\"url\" attr.type=\"string\" for=\"node\" id=\"d4\"/>");
		writer
				.write("<key attr.name=\"description\" attr.type=\"string\" for=\"node\" id=\"d5\"/>");
		writer
				.write("<key for=\"node\" id=\"d6\" yfiles.type=\"nodegraphics\"/>");
		writer
				.write("<key attr.name=\"Description\" attr.type=\"string\" for=\"graph\" id=\"d7\">");
		writer.write("<default/>");
		writer.write("</key>");
		writer
				.write("<key attr.name=\"url\" attr.type=\"string\" for=\"edge\" id=\"d8\"/>");
		writer
				.write("<key attr.name=\"description\" attr.type=\"string\" for=\"edge\" id=\"d9\"/>");
		writer
				.write("<key for=\"edge\" id=\"d10\" yfiles.type=\"edgegraphics\"/>");
		writer.write("<graph edgedefault=\"directed\" id=\"G\">");

		// Class nodes
		//
		for (Element e : l.values()) {
			Rectangle2D r = e.getRectangle(fIncludeAttributes, fIncludeMethods);
			...
			writer
					.write("<y:NodeLabel alignment=\"center\" autoSizePolicy=\"content\" fontFamily=\"Dialog\" fontSize=\"13\" fontStyle=\"bold\" hasBackgroundColor=\"false\" hasLineColor=\"false\" height=\"19.92626953125\" modelName=\"internal\" modelPosition=\"t\" textColor=\"#000000\" visible=\"true\" width=\"91.40087890625\" x=\"47.799560546875\" y=\"3.0\">");
			writer.write(e.clazz.getSimpleName());
			writer.write("</y:NodeLabel>");
			writer
					.write("<y:UML clipContent=\"true\" constraint=\"\" omitDetails=\"false\" stereotype=\"\" use3DEffect=\"false\">");
		...
		for (Element e : l.values()) {
			...
			writer
					.write("<y:Path sx=\"0.0\" sy=\"0.0\" tx=\"0.0\" ty=\"0.0\"/>");
			writer
					.write("<y:LineStyle color=\"#000000\" type=\"line\" width=\"1.0\"/>");
		...
		for (Element e : l.values()) {
			...
			for (int i = 0; i < f.length; i++) {
				...
				if (fCollection)
					writer
							.write("<y:Arrows source=\"circle\" target=\"none\"/>");
				else
					writer
							.write("<y:Arrows source=\"transparent_circle\" target=\"none\"/>");
