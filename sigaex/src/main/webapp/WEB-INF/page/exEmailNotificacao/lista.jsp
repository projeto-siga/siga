...
<script type="text/javascript">
	...
	var excluir = function () {
		...
		location.href='/sigaex/app/expediente/emailNotificacao/excluir?id=' + idExclusao;
	}
</script>
			...
			<div class="card-body">
						...
						<table border="0" class="table table-sm table-striped">
							...
							<tbody>
								<c:forEach var="email" items="${itens}">
									<tr>																		
										<td align="left">
											<c:if test="${not empty email.dpPessoa}">
												<siga:selecionado sigla="${email.dpPessoa.pessoaAtual.nomeAbreviado}" descricao="${email.dpPessoa.pessoaAtual.descricao} - ${email.dpPessoa.pessoaAtual.sigla}" />
											</c:if>
										</td>
										<td>
											<c:if test="${not empty email.dpLotacao}">
												<siga:selecionado sigla="${email.dpLotacao.lotacaoAtual.sigla}" descricao="${email.dpLotacao.lotacaoAtual.descricaoAmpliada}" />
											</c:if>
										</td>	
										<td></td>
										<td>${email.email}</td>	
										<td>
											<c:if test="${not empty email.pessoaEmail}">
												<siga:selecionado sigla="${email.pessoaEmail.pessoaAtual.nomeAbreviado}" descricao="${email.pessoaEmail.pessoaAtual.descricao} - ${email.pessoaEmail.pessoaAtual.sigla}" />
											</c:if>
										</td>
										<td>
											<c:if test="${not empty email.lotacaoEmail}">
												<siga:selecionado sigla="${email.lotacaoEmail.lotacaoAtual.sigla}" descricao="${email.lotacaoEmail.lotacaoAtual.descricaoAmpliada}" />
											</c:if>
										</td>								
										<td align="center" width="5%">									
			 			 					<a href="javascript:excluirGrupo(${email.idEmailNotificacao})">
												<img style="display: inline;" src="/siga/css/famfamfam/icons/cancel_gray.png" title="Excluir email"							
				...
				<div class="row">
					<div class="col-sm-8">
						...
						<button type="button" onclick="javascript:window.location.href='${url}'" class="btn btn-primary">Incluir</button>
					</div>
				</div>
			</div>
		</div>
