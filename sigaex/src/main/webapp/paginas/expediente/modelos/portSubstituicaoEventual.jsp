...
<mod:modelo urlBase="/paginas/expediente/modelos/portaria.jsp">
	<mod:entrevista>
		...
		<mod:grupo>
			<mod:texto titulo="Cargo/Função Comissionada" var="cargofc" />
		</mod:grupo>
	...
	<mod:documento>
		<mod:valor var="texto_portaria">
			...
			cargo em comissão/função comissionada<c:choose><c:when test="${(empty dataInicio) and (empty dataFim)}">.</c:when><c:when test="${dataInicio == dataFim || empty dataFim}">, a partir de ${dataInicio}.</c:when><c:otherwise>, no período de ${dataInicio} a ${dataFim}.</c:otherwise></c:choose>
			</p>
