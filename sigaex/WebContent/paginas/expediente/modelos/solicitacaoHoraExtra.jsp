							...
							<c:forEach var="j" begin="1" end="${requestScope[f:concat('numDatas',i)]}">
								<mod:grupo>
									<mod:data titulo="Data" var="data${i}${j}"></mod:data> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
						    		 <mod:hora titulo="Horário" var="horaIni${i}${j}" /> h às  <mod:hora var="horaFim${i}${j}" titulo=""/> h
								</mod:grupo>
							</c:forEach>
						</mod:grupo>
					</mod:grupo>
				...
				<tr>
					...
					<c:forEach var="j" begin="1"
						...
						<ww:if test="${j == 1}">
							...
							<td bgcolor="#FFFFFF" width="10%">${requestScope[f:concat(f:concat('horaIni',i),j)]} h às 
							${requestScope[f:concat(f:concat('horaFim',i),j)]} h</td>
						</ww:if>
						<ww:else>
							<tr>
								<td bgcolor="#FFFFFF" width="20%" align="center">${f:maiusculasEMinusculas(f:pessoa(servidorX).nomePessoa)}</td>
