package br.gov.jfrj.siga.api.v1;

import java.util.ArrayList;
import java.util.List;

import com.crivano.swaggerservlet.SwaggerException;
import com.crivano.swaggerservlet.SwaggerServlet;

import br.gov.jfrj.siga.api.v1.ISigaApiV1.IPessoasPinResetPost;
import br.gov.jfrj.siga.api.v1.ISigaApiV1.PessoasPinResetPostRequest;
import br.gov.jfrj.siga.api.v1.ISigaApiV1.PessoasPinResetPostResponse;
import br.gov.jfrj.siga.base.RegraNegocioException;
import br.gov.jfrj.siga.cp.CpIdentidade;
import br.gov.jfrj.siga.cp.bl.Cp;
import br.gov.jfrj.siga.dp.DpPessoa;
import br.gov.jfrj.siga.dp.dao.CpDao;
import br.gov.jfrj.siga.vraptor.SigaObjects;

public class PessoasPinResetPost implements IPessoasPinResetPost {


	@Override
	public void run(PessoasPinResetPostRequest req, PessoasPinResetPostResponse resp) throws Exception {
		try (ApiContext ctx = new ApiContext(true, true)) {
			CurrentRequest.set(new RequestInfo(null, SwaggerServlet.getHttpServletRequest(), SwaggerServlet.getHttpServletResponse()));
			
			final String tokenPin = req.tokenPin;
			final String pin = req.pin;
			
			SigaObjects so = ApiContext.getSigaObjects();
			CpIdentidade identidadeCadastrante = so.getIdentidadeCadastrante();
			
			DpPessoa cadastrante = so.getCadastrante();
			
			
			if (Cp.getInstance().getBL().isTokenResetPinValido(cadastrante.getCpfPessoa(),tokenPin)) {		
				if (Cp.getInstance().getBL().consisteFormatoPin(pin)) {

					List<CpIdentidade> listaIdentidades = new ArrayList<CpIdentidade>();
					listaIdentidades = CpDao.getInstance().consultaIdentidadesPorCpf(cadastrante.getCpfPessoa().toString());	
					
					Cp.getInstance().getBL().definirPinIdentidade(listaIdentidades, pin, identidadeCadastrante);
					Cp.getInstance().getBL().invalidarTokenUtilizado(cadastrante.getCpfPessoa(),tokenPin);
					Cp.getInstance().getBL().enviarEmailDefinicaoPIN(cadastrante,"Redefinição Chave PIN","Você redefiniu sua chave PIN.");
					resp.mensagem = "Chave PIN foi trocada.";
					
				}	 
			 } else {
				 throw new RegraNegocioException("Token para reset de PIN inválido ou expirado.");
			 }

		} catch (RegraNegocioException e) {
			throw new SwaggerException(e.getMessage(), 400, null, req, resp, null);
		} catch (Exception e) {
			e.printStackTrace(System.out);
			throw e;
		}
		
	}
	
	@Override
	public String getContext() {
		return "reset chave PIN";
	}
	
}
