...
carga_XMLs(){
	...
	local ThisSigla="${FileName%%.*}"
	local ThisXML="${BASE}/xmls/${ThisSigla}.XML"
	local ThisLogPreCarga="${BASE}/logs/${ThisSigla}_pre_carga.log"
	local ThisLogCarga="${BASE}/logs/${ThisSigla}.log"
...
#   Testa se a carga do arquivo XML será bem sucedida, porem não efetua as atualizações.
    ...
    local ThisExcluidos=`egrep -ic excluindo ${ThisLogPreCarga}`
	local ThisPessoas=`egrep -ic 'pessoa ' ${ThisXML}`
	local ThisLogPreCarga_atualizacoes=`egrep -ic atualizando ${ThisLogPreCarga}`
#   Verifica a carga foi bem sucedida, ou não.
#   Verifica se o layout é de uma carga mesmo ou se é apenas o relatório de erro.
    local certo=`egrep -ic "Total de altera.*es" $ThisLogPreCarga`
    local layout=`egrep -ic "XML confirma .*rg.*o usu.*rio" $ThisLogPreCarga`
...
# 1 informa que é o log do aplicativo de carga (API REST), 0 trata-se do relatório do 'parsing'
    echo -e "${ThisSigla} Layout $layout"
    echo -e "${ThisSigla} ThisExcluidos $ThisExcluidos, ThisPessoas $ThisPessoas, ThisLogPreCarga_atualizacoes $ThisLogPreCarga_atualizacoes"
...
#         efetua a carga propriamente dita	
          ...
          echo -e "${ThisSigla} Arquivo ${ThisXML} processado com sucesso!!!\n"
...
#         echo -e "\n\n\n\nO arquivo ${ThisXML} veio com ${ThisPessoas} pessoas, e na pre-carga constam ${ThisExcluidos} exclusoes, ${ambiente}. \n\n\n O arquivo em tela necessita de conferencia manual.\n" | mail -s "Processo rio, ${ambiente}, alerta de carga com muitas exclusoes. ${ThisSigla}" jborges@rio.rj.gov.br
          ...
          echo -e "${ThisSigla}O arquivo ${ThisXML} nao foi processado pois contem erros!!!\n" 1> ${ThisLogCarga} 2>&1
          nl -bn ${ThisLogCarga}
...
#       mv ${BASE}/xmls/SME.XML ${BASE}/zips/SME.XML_`$now`
        mv ${BASE}/xmls/XXX.XML ${BASE}/zips/XXX.XML_`$now` 2> /dev/null 
        mv ${BASE}/xmls/ServidoresSemUABasica.XML ${BASE}/zips/ServidoresSemUABasica.XML_`$now` 2> /dev/null 
        mv ${BASE}/xmls/ServidoresDemUABasica.XML ${BASE}/zips/ServidoresDemUABasica.XML_`$now` 2> /dev/null 
...
do
	...
	if [[ $(($x % $PARALLEL_DEGREE_LIMIT)) -eq 0 ]]; then
	   wait
	fi
done
...
#   tar -zxvf arquivo_baixado.tar.gz -C /home/user/temp # descompactar    
fi
