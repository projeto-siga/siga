...
alter table corporativo.cp_orgao_usuario add constraint CP_ORG_USU_CP_IDENT_ID_INI_FK foreign key(HIS_IDC_INI) references corporativo.cp_identidade(ID_IDENTIDADE);
alter table corporativo.cp_orgao_usuario add constraint CP_ORG_USU_CP_IDENT_ID_FIM_FK foreign key(HIS_IDC_FIM) references corporativo.cp_identidade(ID_IDENTIDADE);
