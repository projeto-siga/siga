...
<siga:pagina titulo="Lista de Expedientes" popup="${popup}">
	...
	<div id="inicio" class="container-fluid content mb-3">
		<h5 id="pesqTitle">${!(empty fromQuadro or fromQuadro == 'true') ? "Pesquisar Documentos" : "Resultado da Pesquisa"}</h5>
		<c:set var="formOrigem" value="lista" scope="request"/>
		<c:set var="resultadoEmbaixo" value="${((empty apenasRefresh) or (apenasRefresh != 1)) and siga_cliente == 'GOVSP' and (empty fromQuadro or fromQuadro != 'true')}" scope="request"/>
		<c:if
			test="${((empty primeiraVez) or (primeiraVez != 'sim')) and !resultadoEmbaixo}">
			<c:choose>
		...
		<form id="listar" name="listar" action="listar" method="get" class="form100">
			...
			<input type="hidden" name="fromQuadro" value="${fromQuadro}" /> 
			...
			<c:if test="${not ehPublicoExterno}">
				<div class="card bg-light ${pesquisaLimitadaPorData? 'sticky-top':''}">
					<div class="${((!empty fromQuadro and fromQuadro == 'true') and siga_cliente == 'GOVSP') ? 'd-none' : ''}">
