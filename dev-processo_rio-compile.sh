...
if deploy_siga=`/opt/java/jboss-eap-7.2/bin/jboss-cli.sh --connect --command="deployment deploy-file $JBOSS_HOME/standalone/deployments/"`; then
...
echo "SIGAEX:"
if deploy_sigaex=`/opt/java/jboss-eap-7.2/bin/jboss-cli.sh --connect --command="deployment deploy-file $JBOSS_HOME/standalone/deployments/"`; then
