	...
	<div class="container-fluid">
		...
		<div class="row">
						...
						<table border="0" class="table table-sm">
							...
							<tbody>
								<c:forEach var="i" items="${listaMaisRecentes}">
									<tr class="gt-td">
										<td width="45%"><a
											href="${linkTo[AppController].exibir(i.siglaCompacta)}">${i.arq.titulo}</a></td>
			...
			<div class="col col-md-6">
							...
							<tbody>
								<c:forEach items="${listaPrincipaisLotacoes}" var="l">
									<tr>
										...
										<td style="width: 20%; text-align: right;"><a
											href="${linkTo[AppController].listar}?filtro.lotacao.id=${l[1]}&filtro.situacao.idMarcador=36&filtro.pesquisa=true">${l[3]}</a></td>
									</tr>
								</c:forEach>
							</tbody>
						</table>
				...
				<c:if test="${empty listaPrincipaisLotacoes}">
					<h6 style="background: #d8d8c0; padding: 3px 10px;">Nenhuma
						Lotação.</h6>
				</c:if>
				<br />
				<h2>Evolução</h2>
				<div id="evolution" class=""></div>
