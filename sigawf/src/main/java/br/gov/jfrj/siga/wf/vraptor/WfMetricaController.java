...
@Controller
public class WfMetricaController extends WfController {
	 ...
	 */
	public WfMetricaController() {
		super();
	}

	public WfMetricaController(HttpServletRequest request, Result result, WfDao dao, SigaObjects so, WfUtil util) {
	 ...
	 * @throws Exception
	 */
	public void medir(Long orgao, String procedimento, Long pdId) throws Exception {
...
//				SigaWfProperties.getRelEstatGeraisMinMediaTrunc().toString().replace(".", ","));
//		result.include("maxMediaTruncada",
//				SigaWfProperties.getRelEstatGeraisMaxMediaTrunc().toString().replace(".", ","));
...
//		parametrosRelatorio.put("nomeProcedimento", WfDao.getInstance().getProcessDefinition(pdId).getName());
