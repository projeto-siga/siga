...
<siga:pagina titulo="Transferência em Lote">
	...
	function checkUncheckAll(theElement) {
		...
		for(z=0; z<theForm.length;z++) {
	    	if(theForm[z].type == 'checkbox' && theForm[z].name != 'checkall') {
				...
				theForm[z].click();
			} else if(((theForm[z].type == 'select-one' && theForm[z].name.substr(0,4) == 'tpd_') || (theForm[z].type == 'text' && theForm[z].name.substr(0,4) == 'txt_')) && theForm[z].name != 'tpdall' && theForm[z].name != 'txtall'){
	...
	function displaySel(chk, el) {
		...
		else enableDisableItem(chk.name.substring(4), false);
	}
	...
	function enableDisableAll(theElement) {
		var theForm = theElement.form, z = 0;
		for(z=0; z<theForm.length; z++) {
			if(((theForm[z].type == 'select-one' && theForm[z].name.substr(0,4) == 'tpd_') || (theForm[z].type == 'text' && theForm[z].name.substr(0,4) == 'txt_')) && theForm[z].name != 'tpdall' && theForm[z].name != 'txtall'){
				if (theElement.value != 0)
					enableDisableItem(theForm[z].name.substring(4), false);
				else enableDisableItem(theForm[z].name.substring(4), true);
			}
	...
	<ww:form name="frm" action="transferir_lote_gravar"
		...
		<table class="gt-form-table">
			...
			<tr>
				...
				<td>
				<div id="div_tpdall"><ww:select name="tpdall" id="tpdall"
					onchange="javascript:enableDisableAll(this);" list="tiposDespacho"
					listKey="idTpDespacho" listValue="descTpDespacho" /></div>
			...
			<tr>
				<td></td>
				<td>
				<div id="div_txtall" style="display: none"><input type="text"
						...
						<table class="gt-table">
							...
							<tr class="${evenorodd}">
								...
								<td width="2%" align="center"><input type="checkbox"
									name="${x}" value="true"
									${x_checked} onclick="javascript:displaySel(this, '${tpd_x}');" /></td>
								<td><c:remove var="style" /><c:if test="${empty param[x]}">
									<c:set var="style" value=" style=display:none" />
								</c:if>
								<div id="div_${tpd_x}"${style}><select name="${tpd_x}"
									id="${tpd_x}"
								...
								</select> <c:remove var="style" /><c:if test="${param[tpd_x] != -1}">
									<c:set var="style" value=" style=display:none" />
								</c:if>
								<div id="div_${txt_x}"${style}><input type="text"
									name="${txt_x}" id="${txt_x}" value="${param[txt_x]}"
									maxlength="255" /></div>
								</div>
