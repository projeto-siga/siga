		...
		<c:if
			...
			<c:choose>
				<c:when test="${listEstado[0]==1}">
					<c:set var="titulo1" value="Documentos que estão em estado temporário(rascunho) e ainda podem ser editados."></c:set>
					<c:set var="titulo2" value="Documentos que estão em estado temporário(rascunho) e ainda podem ser editados."></c:set>
					<c:set var="titulo3" value="${titulo1}"></c:set>
				</c:when>
				<c:when test="${listEstado[0]==2}">
					<c:set var="titulo1" value="Documentos que já foram assinados ou tiveram a assinatura manual registrada. Também contém os documentos que já foram recebidos pela lotação ${lotaTitular.nomeLotacao} ou pelo usuário ${titular.nomePessoa}."></c:set>
					<c:set var="titulo2" value="Documentos que já foram assinados ou tiveram a assinatura manual registrada. Também contém os documentos que já foram recebidos pelo usuário ${titular.nomePessoa}."></c:set>
					<c:set var="titulo3" value="${titulo1}"></c:set>
				</c:when>
				<c:when test="${listEstado[0]==3}">
					<c:set var="titulo1" value="Documentos não eletrônicos transferidos para a ${lotaTitular.nomeLotacao} ou para o usuário ${titular.nomePessoa} que estão aguardando recebimento."></c:set>
					<c:set var="titulo2" value="Documentos não eletrônicos transferidos para o usuário ${titular.nomePessoa} que estão aguardando recebimento."></c:set>
					<c:set var="titulo3" value="${titulo1}"></c:set>
				</c:when>
				<c:when test="${listEstado[0]==14}">
					<c:set var="titulo1" value="Documentos eletrônicos transferidos para a ${lotaTitular.nomeLotacao} ou para o usuário ${titular.nomePessoa} que estão aguardando recebimento."></c:set>
					<c:set var="titulo2" value="Documentos eletrônicos transferidos para o usuário ${titular.nomePessoa} que estão aguardando recebimento."></c:set>
					<c:set var="titulo3" value="${titulo1}"></c:set>
				</c:when>
				<c:when test="${listEstado[0]==15}">
					<c:set var="titulo1" value="Documentos que foram finalizados mas ainda não foram assinados."></c:set>
					<c:set var="titulo2" value="Documentos que foram finalizados mas ainda não foram assinados."></c:set>
					<c:set var="titulo3" value="${titulo1}"></c:set>
				</c:when>
				<c:when test="${listEstado[0]==23}">
					<c:set var="titulo1" value="Documentos não eletrônicos que foram transferidos, pela lotação ${lotaTitular.nomeLotacao} ou pelo usuário ${titular.nomePessoa}, para outra lotação/pessoa mas ainda não foram recebidos."></c:set>
					<c:set var="titulo2" value="Documentos não eletrônicos que foram transferidos, pelo usuário ${titular.nomePessoa}, para outra lotação/pessoa mas ainda não foram recebidos."></c:set>
					<c:set var="titulo3" value="${titulo1}"></c:set>
				</c:when>
				<c:when test="${listEstado[0]==24}">
					<c:set var="titulo1" value="Documentos eletrônicos que foram transferidos, pela lotação ${lotaTitular.nomeLotacao} ou pelo usuário ${titular.nomePessoa}, para outra lotação/pessoa mas ainda não foram recebidos."></c:set>
					<c:set var="titulo2" value="Documentos eletrônicos que foram transferidos, pelo usuário ${titular.nomePessoa}, para outra lotação/pessoa mas ainda não foram recebidos."></c:set>
					<c:set var="titulo3" value="${titulo1}"></c:set>
				</c:when>
				<c:when test="${listEstado[0]==25}">
					<c:set var="titulo1" value="Documentos pendentes de assinatura cujo subscritor é o usuário ${titular.nomePessoa}."></c:set>
			...
			<tr>
				...
				</ww:url><siga:monolink
							titulo="${titulo1}"
							texto="${listEstado[1]}" href="%{url}" />
				...
				<td align="right" class="count"><ww:url id="url"
					...
					titulo="${titulo3}"
					texto="${listEstado[3]}" href="%{url}" /> <%--<ww:a href="%{url}" title="${titulo1}">${listEstado[3]}</ww:a>--%></td>
			</tr>
		</c:if>
	</c:forEach>
</table>
	...
	<table width="100%" class="zero">
		<tr>
			...
			<siga:selecao tema="simple" buscar="nao" propriedade="documentoVia"
				ocultardescricao="sim" /> <input type="button" name="ok" value="Ok"
