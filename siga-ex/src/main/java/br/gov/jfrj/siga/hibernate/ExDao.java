...
public class ExDao extends CpDao {
        ...
        do {
            ...
            for (ExDocumento doc : list) {
                ...
                if (doc.isIndexavel()
                        ...
                        new TermQuery(new Term("idDoc",String.valueOf(doc.getIdDoc()))),
                        ExDocumento.class).getResultSize() == 0)) {

                    if (irIndexando) {
                        log.info("listarNaoIndexados - indexando " + doc.getCodigo());
                        indexar(doc);
                    } else {
                        log.info("listarNaoIndexados - nao indexar " + doc.getCodigo());
                    }
                }
            }
            log.info("listarNaoIndexados - " + firstResult + " varridos");
            getSessao().clear();
        } while (list.size() > 0);
        log.info("listarNaoIndexados - FIM    "
        ...
        try {
            ...
            getSessao().doWork(new Work() {
                ...
                public void execute(Connection conn) throws SQLException {
                    ...
                    try {
                        psBlob = conn.prepareStatement(s);
    ...
    public void gravarPreenchimentoBlob(final ExPreenchimento exPreenchimento) throws SQLException, AplicacaoException {
        getSessao().flush();
        getSessao().doWork(new Work() {
            @Override
            public void execute(Connection conn) throws SQLException {
    ...
    public ExPreenchimento consultarPreenchimentoBlob(final ExPreenchimento exPreenchimento) throws SQLException {
        ...
        getSessao().doWork(new Work() {
            ...
            public void execute(Connection conn) throws SQLException {
                ...
                // TODO se ocorrer uma exceção isso nunca será fechado.
                psBlob.close();
                rset.close();
            }
        });
    ...
    static private void configurarHibernate(Configuration cfg)throws Exception {
        ...
        cfg.setCacheConcurrencyStrategy("br.gov.jfrj.siga.ex.ExFormaDocumento","read-only", CACHE_EX);
        ...
        // cfg.setCacheConcurrencyStrategy("br.gov.jfrj.siga.ex.ExVia", "read-only", "ex");
        cfg.setCacheConcurrencyStrategy("br.gov.jfrj.siga.ex.ExTipoMobil", "read-only", CACHE_EX);
        cfg.setCollectionCacheConcurrencyStrategy("br.gov.jfrj.siga.ex.ExTipoDocumento.exFormaDocumentoSet", "read-only", CACHE_EX);
        cfg.setCacheConcurrencyStrategy("br.gov.jfrj.siga.dp.CpTipoMarca", "nonstrict-read-write", "corporativo");
        cfg.setCacheConcurrencyStrategy("br.gov.jfrj.siga.dp.CpTipoMarcador", "nonstrict-read-write", "corporativo");
