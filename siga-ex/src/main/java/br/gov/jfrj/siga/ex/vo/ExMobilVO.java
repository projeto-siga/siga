...
public class ExMobilVO extends ExVO {
	...
	public ExMobilVO(ExMobil mob, DpPessoa titular, DpLotacao lotaTitular, boolean completo) throws Exception {
		this(mob, titular, lotaTitular, completo, null, false);
	}
	
	public ExMobilVO(ExMobil mob, DpPessoa titular, DpLotacao lotaTitular, boolean completo, Long tpMov, boolean movAssinada)
			throws Exception {
		this.mob = mob;
		...
		/*
		 * Markenson: O código abaixo foi comentado por questões de desempenho. Deve ser estudada uma maneira mais eficiente de
		 * calcular o tamanho dos PDFs
		 */
		...
		System.out.println(mob.getExDocumento().getCodigoString() + ": aExibir - mobil " + mob.getNumSequencia()
		...
		for (ExDocumento d : mob.getExDocumentoFilhoSet()) {
			...
			if (!d.isFinalizado() || !d.isCancelado()) {
				if (d.isExpediente())
					expedientesFilhosNaoCancelados.add(new ExDocumentoVO(d, null, titular, lotaTitular, false, false));
				else
					processosFilhosNaoCancelados.add(new ExDocumentoVO(d, null, titular, lotaTitular, false, false));
			}
		}
		
		for (ExDocumento doc : mob.getDocsFilhosNaoJuntados())
			expedientesFilhosNaoJuntados.add(new ExDocumentoVO(doc, null, titular, lotaTitular, false, false));
		...
		if (mob.doc().isEletronico()) {
			for (ExMovimentacao mov : mob.getAnexosNaoAssinados())
				anexosNaoAssinados.add(new ExMovimentacaoVO(this, mov, titular, lotaTitular));
			
			for (ExMovimentacao mov : mob.getDespachosNaoAssinados())
				despachosNaoAssinados.add(new ExMovimentacaoVO(this, mov, titular, lotaTitular));
		}
		
		if (mob.getPendenciasDeAnexacao() != null) {
			for (ExMovimentacao mov : mob.getPendenciasDeAnexacao())
				pendenciasDeAnexacao.add(new ExMovimentacaoVO(this, mov, titular, lotaTitular));
		}
		...
		for (ExMovimentacaoVO movVO : movs) {
			...
			if (i == movs.size()
					|| (movVO.mov.getExTipoMovimentacao().getId().equals(ExTipoMovimentacao.TIPO_MOVIMENTACAO_RECEBIMENTO) && (!movVO
							.isCancelada()))) {
				if (i == movs.size()) {
					duracaoSpan++;
				}
				if (i == movs.size() || movVOUlt == null)
					movVOUlt = movVO;
				SigaCalendar lIni = new SigaCalendar(movVOIni.mov.getDtIniMov().getTime());
				SigaCalendar lFim = new SigaCalendar(movVOUlt.mov.getDtIniMov().getTime());
	 ...
	 * @throws Exception
	 */
	private void addAcoes(ExMobil mob, DpPessoa titular, DpLotacao lotaTitular) throws Exception {
		if (!mob.isGeral()) {
			addAcao("application_side_tree", "Visualizar Dossiê", "/app/expediente/doc", "exibirProcesso", Ex.getInstance()
					.getComp().podeVisualizarImpressao(titular, lotaTitular, mob), null, null, null, null, "once");
			
			addAcao("printer", "Visualizar Impressão", "/app/arquivo", "exibir", Ex.getInstance().getComp()
					.podeVisualizarImpressao(titular, lotaTitular, mob), null, "&popup=true&arquivo=" + mob.getReferenciaPDF(),
					null, null, null);
			
			addAcao("link_add", "Criar Anexo", "/expediente/doc", "editar", Ex.getInstance().getComp()
					.podeAnexarArquivoAlternativo(titular, lotaTitular, mob), null, "criandoAnexo=true&mobilPaiSel.sigla="
					+ getSigla(), null, null, null);
		}
		addAcao("link_break", "Desentranhar", "/expediente/mov", "cancelar_juntada", Ex.getInstance().getComp()
				.podeCancelarJuntada(titular, lotaTitular, mob), null, null, null, null, "once");
		
		addAcao("link_delete", "Desapensar", "/expediente/mov", "desapensar",
				Ex.getInstance().getComp().podeDesapensar(titular, lotaTitular, mob), null, null, null, null, "once");
		
		addAcao("email_open", "Receber", "/expediente/mov", "receber",
				Ex.getInstance().getComp().podeReceber(titular, lotaTitular, mob), null, null, null, null, "once");
		
		addAcao("email_edit", "Despachar/Transferir", "/app/expediente/mov", "transferir", Ex.getInstance().getComp()
				.podeDespachar(titular, lotaTitular, mob)
				|| Ex.getInstance().getComp().podeTransferir(titular, lotaTitular, mob), null, "&popup=true", null, null, null);
		
		if (mob.isVia() || mob.isVolume()) {
			addAcao("attach", "Anexar Arquivo", "/expediente/mov", "anexar",
					Ex.getInstance().getComp().podeAnexarArquivo(titular, lotaTitular, mob));
			addAcao("tag_yellow", "Fazer Anotação", "/app/expediente/mov", "anotar", Ex.getInstance().getComp()
					.podeFazerAnotacao(titular, lotaTitular, mob));
		}
		
		addAcao("package", "Arq. Corrente", "/expediente/mov", "arquivar_corrente_gravar", Ex.getInstance().getComp()
				.podeArquivarCorrente(titular, lotaTitular, mob), null, null, null, null, "once");
		
		addAcao("database_add", "Indicar para Guarda Permanente", "/expediente/mov", "indicar_permanente", Ex.getInstance()
				.getComp().podeIndicarPermanente(titular, lotaTitular, mob));
		
		addAcao("database_delete", "Reverter Ind. Guarda Permanente", "/expediente/mov", "reverter_indicacao_permanente", Ex
				.getInstance().getComp().podeReverterIndicacaoPermanente(titular, lotaTitular, mob));
		
		addAcao("page_red", "Retirar de Edital de Eliminação", "/expediente/mov", "retirar_de_edital_eliminacao", Ex
				.getInstance().getComp().podeRetirarDeEditalEliminacao(titular, lotaTitular, mob));
		
		addAcao("table_edit", "Reclassificar", "/expediente/mov", "reclassificar",
				Ex.getInstance().getComp().podeReclassificar(titular, lotaTitular, mob));
		
		addAcao("table", "Avaliar", "/expediente/mov", "avaliar",
				Ex.getInstance().getComp().podeAvaliar(titular, lotaTitular, mob));
		
		addAcao("hourglass_add", "Sobrestar", "/expediente/mov", "sobrestar_gravar",
				Ex.getInstance().getComp().podeSobrestar(titular, lotaTitular, mob), null, null, null, null, "once");
		
		addAcao("database", "Recolher ao Arq. Permanente", "/expediente/mov", "arquivar_permanente_gravar", Ex.getInstance()
				.getComp().podeBotaoArquivarPermanente(titular, lotaTitular, mob), null, null, null, null, "once");
		
		addAcao("box", "Arq. Intermediário", "/expediente/mov", "arquivar_intermediario", Ex.getInstance().getComp()
				.podeBotaoArquivarIntermediario(titular, lotaTitular, mob), null, null, null, null, "once");
		
		addAcao("package_go", "Desarq. Corrente", "/expediente/mov", "reabrir_gravar", Ex.getInstance().getComp()
				.podeDesarquivarCorrente(titular, lotaTitular, mob), null, null, null, null, "once");
		
		addAcao("box_go", "Desarq. Intermediário", "/expediente/mov", "desarquivar_intermediario_gravar", Ex.getInstance()
				.getComp().podeBotaoDesarquivarIntermediario(titular, lotaTitular, mob), null, null, null, null, "once");
		
		addAcao("hourglass_delete", "Desobrestar", "/expediente/mov", "desobrestar_gravar", Ex.getInstance().getComp()
				.podeDesobrestar(titular, lotaTitular, mob), null, null, null, null, "once");
		
		addAcao("link", "Juntar", "/expediente/mov", "juntar", Ex.getInstance().getComp().podeJuntar(titular, lotaTitular, mob));
		
		addAcao("page_find", "Vincular", "/app/expediente/mov", "referenciar",
				Ex.getInstance().getComp().podeReferenciar(titular, lotaTitular, mob));
		
		addAcao("link_add", "Apensar", "/expediente/mov", "apensar",
		...
		if (mob.getExDocumento().isFinalizado()
				...
				&& mob.getUltimaMovimentacaoNaoCancelada().getExTipoMovimentacao().getIdTpMov() != ExTipoMovimentacao.TIPO_MOVIMENTACAO_INCLUSAO_DE_COSIGNATARIO)
			addAcao("arrow_undo", "Desfazer " + mob.getDescricaoUltimaMovimentacaoNaoCancelada(), "/expediente/mov",
					"cancelarMovimentacao", Ex.getInstance().getComp().podeCancelarMovimentacao(titular, lotaTitular, mob),
					"Confirma o cancelamento da última movimentação(" + mob.getDescricaoUltimaMovimentacaoNaoCancelada() + ")?",
					null, null, null, "once"); // popup,
		// exibir+completo,
		// confirmacao
		
		addAcao("folder_page_white", "Encerrar Volume", "/expediente/mov", "encerrar_volume_gravar", Ex.getInstance().getComp()
				.podeEncerrarVolume(titular, lotaTitular, mob), "Confirma o encerramento do volume?", null, null, null, "once");
		
		addAcao("cancel", "Cancelar Via", "/expediente/mov", "cancelarMovimentacao",
				Ex.getInstance().getComp().podeCancelarVia(titular, lotaTitular, mob), "Confirma o cancelamento da via?", null,
				null, null, "once");
		
		addAcao("brick", "Autuar", "/expediente/doc", "editar", Ex.getInstance().getComp().podeAutuar(titular, lotaTitular, mob),
				null, "idMobilAutuado=" + mob.getId() + "&autuando=true", null, null, null);
	}
	...
	public String getMarcadoresEmHtml(DpPessoa pess, DpLotacao lota) {
		...
		for (ExMarca mar : getMarcasAtivas()) {
			if (mar.getCpMarcador().getIdMarcador() != CpMarcador.MARCADOR_EM_TRANSITO
					...
					&& ((mar.getDpLotacaoIni() != null && lota.getIdInicial().equals(mar.getDpLotacaoIni().getIdInicial())) || mar
							.getDpLotacaoIni() == null)
					&& (mar.getDpPessoaIni() == null || pess.getIdInicial().equals(mar.getDpPessoaIni().getIdInicial()))) {
				if (sb.length() > 0)
					sb.append(", ");
		...
		if (sb.length() == 0) {
			for (ExMarca mar : getMarcasAtivas()) {
				if (mar.getCpMarcador().getIdMarcador() != CpMarcador.MARCADOR_EM_TRANSITO
					...
					if (sb.length() > 0)
						sb.append(", ");
					if ((mar.getDpLotacaoIni() != null && lota.getIdInicial().equals(mar.getDpLotacaoIni().getIdInicial()))
							&& (mar.getDpPessoaIni() != null && !pess.getIdInicial().equals(mar.getDpPessoaIni().getIdInicial()))) {
						sb.append(mar.getCpMarcador().getDescrMarcador());
	...
	public String getDescricaoCompletaEMarcadoresEmHtml(DpPessoa pess, DpLotacao lota) {
	...
	public boolean isPendencias() {
		return anexosNaoAssinados.size() > 0 || despachosNaoAssinados.size() > 0 || expedientesFilhosNaoJuntados.size() > 0
