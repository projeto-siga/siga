...
<%@ attribute name="funcaoBuscar" required="false" %>
...
self.retorna_${propriedade}${tipoSel} = function(id, sigla, descricao) {
	...
	<c:if test="${not empty funcaoBuscar}">
		${funcaoBuscar};
	</c:if> 
}
...
<c:choose>
 ...
 <c:if test="${empty onblur && not empty onchange}">
...
<input type="text" name="${inputNameTipoSel}.sigla" value="<c:out value="${requestScope[propriedadeTipoSel].sigla}"/>" id="formulario_${inputNameTipoSel}_sigla" 
	onkeypress="return handleEnter(this, event)" ${requiredValue} size="25"
