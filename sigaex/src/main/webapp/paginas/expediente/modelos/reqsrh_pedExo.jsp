...
<mod:modelo>
	<mod:entrevista>
			...
			<br/>
			<mod:grupo titulo="">
					<mod:data titulo="Data da Exoneração" var="dtinicio"/>
			</mod:grupo>
			<br/>
		    ...
		    <mod:caixaverif titulo="Carteira(s) do Plano de Saúde do Titular e dependentes <br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;(se a exoneração acontecer antes do término do mês, 
			 ...
			 &nbsp;&nbsp;
			 <mod:selecao titulo="&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Quantidade de dependentes" var="dependente" opcoes=" ;1 (um) dependente;2 (dois) dependentes;3 (três) dependentes;4 (quatro) dependentes;5 (cinco) dependentes;5 (cinco) dependentes;6 (seis) dependentes;7 (sete) dependentes;8 (oito) dependentes;9 (nove) dependentes;10 (dez) dependentes" reler="sim"/>
		     <br>
	...
	margin-bottom: 1cm;
		...
		<c:import url="/paginas/expediente/modelos/inc_cabecalhoCentralizadoPrimeiraPagina.jsp" />
				...
				<td width="100%">
					<table width="100%">
						<tr><BR>
						<td align="center"><p style="font-family:Arial;font-weight:bold;font-size:9pt;">SOLICITAÇÃO DE EXONERAÇÃO</p></td>
						</tr>
					</table>
				</td>
			</tr>
		...
		<c:set var="opt" value="${f:classNivPadr(doc.subscritor.padraoReferencia)}"/>
         <BR>   
			<p style="text-align: center;font-weight:bold;font-size:7pt;"><center><b> ${excel} PRESIDENTE DO TRIBUNAL REGIONAL FEDERAL DA 2ª REGIÃO</b></center></p>
			...
			ramal ${ramal}, vem requerer a Vossa Excelência sua <b>exoneração</b> do cargo que ora ocupa, a partir do dia ${dtinicio}, com base no art. 34, da Lei nº 8.112/90.<br/>
						
			
			<br />
			Esclarece, ainda, estar ciente da diferença entre os institutos da <b>Exoneração</b> e <b>Declaração de Vacância</b>: que
			a <b>Exoneração</b> acarreta a <b> perda do vínculo </b> com a Administração Pública, com o consequente pagamento de indenização de férias
			e que na <b>Declaração de Vacância</b>, adotada nos casos de posse em outro cargo público inacumulável, permanece o vínculo, 
			o que possibilita a <b>averbação  de tempo para férias</b> (sem pagamento de indenização de férias) e a<b> manutenção de determinadas vantagens pessoais</b>,
			desde que o regime jurídico permaneça o mesmo.   
			<br /><br />
			...
			*&nbsp;
			<c:if test="${ certdigi == 'Sim'}">
			    [X] Instrumento de Certificação Digital (smartcard ou similar).
			    <br>		
			</c:if>
			<c:if test="${ certdigi == 'Nao'}">
			    [&nbsp;&nbsp;] Instrumento de Certificação Digital (smartcard ou similar).
			    <br>		
			</c:if>
			&nbsp;&nbsp;&nbsp;
			...
			<c:choose>
					<c:when test="${cartplano == 'Sim' }"> 
						...
						<c:if test="${dependente != ' ' }">
			    			[X] Carteira do Plano de Saúde do Titular e de ${dependente}
						</c:if>
					</c:when>
					<c:otherwise>
						<c:if test="${ cartplano == 'Nao'}">
			    			[&nbsp;&nbsp;] Carteira(s) do Plano de Saúde do Titular e Dependentes
						</c:if>
					</c:otherwise>
			</c:choose>
			<br>&nbsp;&nbsp;&nbsp;
		 ...
		 * Documentos <b>imprescindíveis</b> para instrução do processo administrativo<br>&nbsp;&nbsp;(Resolução nº 148, 
		de 26/05/95, do Conselho da Justiça Federal)
		<br>  
