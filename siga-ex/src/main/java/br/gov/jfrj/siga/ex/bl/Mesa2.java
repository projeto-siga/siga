...
public class Mesa2 {
	...
	private static List<MesaItem> listarReferencias(
		...
		for (ExMobil mobil : references) {
			...
			try {
				...
				offsetItem++;
				ExMovimentacao ultMovTramite = null;
	
				if (mobil.doc().getSubscritor() != null
						&& mobil.doc().getSubscritor().getLotacao() != null) {
					if (SigaMessages.isSigaSP()) {
						ultMovTramite = mobil.getUltimaMovimentacaoDePosse();
						if (ultMovTramite != null) 
							ultMovTramite = mobil
						...
						if (ultMovTramite != null) {
							r.origem = ultMovTramite.getLotacao()
									.getOrgaoUsuario().getSigla() + " / "
									+ ultMovTramite.getLotacao().getSigla();
						} else {
				...
				if (mobil.doc().getSubscritor() != null
					...
					if (SigaMessages.isSigaSP()) {
						if (ultMovTramite != null && ultMovTramite.getLotacao() != null) {
							r.lotaCadastrante = ultMovTramite.getLotacao().getSigla();
						} else {
	...
	public static List<GrupoItem> getMesa(final boolean contar, final Integer qtd, final Integer offset, final DpPessoa titular, final Map<String, SelGrupo> selGrupos, 
		...
		long tempoBanco = System.nanoTime() - tempoIni;
		System.out.println("getMesa - getContadores: " + tempoBanco / 1000000 + " ms ==> ");
		...
		for (CpMarcadorGrupoEnum grpEnum : lstGrupos) {
			...
			long tempoCount = System.nanoTime() - tempoIni;
			System.out.println("getMesa - getMobils: " + tempoCount / 1000000 + " ms ==> ");
			...
			Map<Integer, List<ExMobil>> hashMobGrp = l.stream()
			        .collect(Collectors.groupingBy(k -> (((BigDecimal) k[0]).intValue()), 
			                Collectors.mapping(v -> dao.consultar(Long.valueOf(v[1].toString()), ExMobil.class, false), 
			                		Collectors.toList())));
			System.out.println("getMesa - getMobils: " + (System.nanoTime() - tempoIni) / 1000000 + " ms ==> ");
			tempoIni = System.nanoTime();
