...
<siga:pagina titulo="Movimentação" compatibilidade="IE=EmulateIE9">
				...
				<div class="card-body">
					<form action="anexar_gravarmultiplos" method="post" enctype="multipart/form-data" class="form">
						...
						<div class="row">
							<div class="col-md-2 col-sm-3">
								<div class="form-group">
									...
									<input class="form-control" type="text" name="dtMovString" value="${dtMovString}" onblur="javascript:verifica_data(this,0);" />
								</div>
							</div>
							<div class="col-sm-6">
								<div class="form-group">
							...
							<div class="col-sm-2 mt-4">
								<div class="form-check form-check-inline">
									<input class="form-check-input" type="checkbox" theme="simple" name="substituicao" value="${substituicao}" onclick="javascript:displayTitular(this);" />
						...
						<div class="row">
							<div class="col-sm-6">
								<div class="form-group">
									<div id="tr_titular" style="${!substituicao == '' ? 'display: none' : ''}">
										<label>Titular</label>
							...
							<tr>
								<td colspan="2">
									<div class="gt-form">
										...
										<c:forEach var="mov" items="${mobilCompletoVO.movs}">
											<c:if test="${(not mov.cancelada) and (mov.idTpMov eq 57)}">
												<label class="gt-form-element-label">
													<input type="checkbox" class="gt-form-checkbox" name="pendencia_de_anexacao" value="${mov.idMov}">
													${mov.descricao}
												</label>
											</c:if>
										</c:forEach>
						...
						<div class="row">
							<div class="col-sm">
								...
								<input type="button" value="Cancela"
									...
									class="btn btn-cancel ml-2" /><input class="ml-2" type="checkbox"
									name="check"
					...
					<thead class="${thead_color}">
						...
						<tr>
							<th style="text-align: center"><input type="checkbox"
								name="checkall" onclick="checkUncheckAll(this)" checked /></th>
						...
						<c:if test="${(not mov.cancelada)}">
							<tr class="${mov.classe} ${mov.disabled}">
								...
								<td style="text-align: center"><input type="checkbox"
									name="${x}" value="true" checked ${x_checked} /></td>
	...
	<div id="dados-assinatura" style="display: none">
		...
		<c:set var="podeAssinarComSenha"
			value="${f:podeAssinarMovimentacaoDoMobilComSenha(titular,lotaTitular,mob)}" />
		<c:set var="podeAutenticarComSenha"
			value="${f:podeAutenticarComSenha(titular,lotaTitular,mob)}" />
		<c:set var="defaultAssinarComSenha"
			value="${f:deveAssinarComSenha(titular,lotaTitular,mob) }" />
		<c:set var="defaultAutenticarComSenha"
			value="${f:deveAutenticarComSenha(titular,lotaTitular,mob) }" />
