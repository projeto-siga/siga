...
public class ExDocumentoAction extends ExActionSupport {
		...
		if (getPar().get("obrigatorios") != null)
			for (String valor : getPar().get("obrigatorios"))
				if (getPar().get(valor) == null
						...
						|| getPar().get(valor)[0].trim().equals("N�o")
	...
	public String aCriarVia() throws Exception {
		...
		if (!Ex.getInstance().getComp()
			...
			throw new AplicacaoException(
					"N�o � poss�vel criar vias neste documento");
		try {
			Ex.getInstance().getBL()
	...
	public String aCriarVolume() throws Exception {
		...
		if (!Ex.getInstance().getComp()
			...
			throw new AplicacaoException(
					"N�o � poss�vel criar volumes neste documento");
		try {
			Ex.getInstance().getBL()
	...
	public String aExcluir() throws Exception {
		...
		try {
			...
			throw new AplicacaoException(
					"Documento j� foi exclu�do anteriormente");
	...
	private void verificaDocumento() throws AplicacaoException, Exception {
		if ((doc.getSubscritor() == null && doc.getNmSubscritor() == null && doc
				...
				&& ((doc.getExFormaDocumento().getExTipoFormaDoc()
						...
						.getIdTipoFormaDoc() != 2))
			throw new AplicacaoException(
					"� necess�rio definir um subscritor para o documento.");
		...
		if (doc.getExClassificacao() == null)
			throw new AplicacaoException(
					"� necess�rio informar a classifica��o documental.");
		...
		try {
			...
			if (!Ex.getInstance()
				...
				if (!Ex.getInstance()
					...
					throw new AplicacaoException(
							"Usu�rio n�o possui permiss�o de criar documento da classifica��o "
			...
			if (isClassificacaoIntermediaria()
					&& (descrClassifNovo == null || descrClassifNovo.trim()
							.length() == 0))
				throw new AplicacaoException(
						"Quando a classifica��o selecionada n�o traz informa��o para cria��o de vias, o sistema exige que, antes de gravar o documento, seja informada uma sugest�o de classifica��o para ser inclu�da na pr�xima revis�o da tabela de classifica��es.");

			if (doc.getDescrDocumento().length() > getTamanhoMaximoDescricao())
				throw new AplicacaoException(
						"O campo descri��o possui mais do que "
			...
			try {
				...
				throw new AplicacaoException(
						"Erro ao tentar incluir os cosignat�rios deste documento",
						0, e);
	...
	public String aDesfazerCancelamentoDocumento() throws Exception {
		...
		if (!Ex.getInstance()
				...
				.podeDesfazerCancelamentoDocumento(getTitular(),
						getLotaTitular(), mob))
			throw new AplicacaoException(
					"N�o � poss�vel desfazer o cancelamento deste documento");
		try {
			Ex.getInstance()
					.getBL()
	...
	public String aTornarDocumentoSemEfeitoGravar() throws Exception {
		if (getDescrMov() == null || getDescrMov().trim().length() == 0) {
			throw new AplicacaoException(
					"O preenchimento do campo MOTIVO � obrigat�rio!");
		}
		buscarDocumento(true);
		if (!Ex.getInstance()
				.getComp()
				.podeTornarDocumentoSemEfeito(getTitular(), getLotaTitular(),
						mob))
			throw new AplicacaoException(
					"N�o � poss�vel tornar documento sem efeito.");
		try {
			Ex.getInstance()
					.getBL()
