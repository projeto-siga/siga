...
public class GiRelatorioAction extends SigaActionSupport implements Preparable {
	...
	// Parâmetros de entrada para para o Relatório de Acesso à Utilização de Serviço
	private String idServico; 
	private String situacoesSelecionadas ; 
	private String idOrgaoUsuario;
	//
	private String idPessoa; // Parâmetro de entrada para para os Relatórios de Permissão e de histórico
	private String dataInicio; // Parâmetro de entrada para para o Relatório de Alteração de Direitos 
	private String dataFim; // Parâmetro de entrada para para o Relatório de Alteração de Direitos
	// preparação
	...
	public String aEmitirRelAcessoServico() throws Exception {
		...
		listaParametros.put("situacoesSelecionadas",getSituacoesSelecionadas());
		listaParametros.put("idOrgaoUsuario",getIdOrgaoUsuario());
		/*  === Verificação dôs parâmetros === 
		 Enumeration params = 
			  ServletActionContext.getRequest().getParameterNames();
		while (params.hasMoreElements()) {
			String oPar = (String) params.nextElement();
			if (oPar.startsWith("situacao_")) {
				listaParametros.put(oPar,(String) (ServletActionContext.getRequest().getParameter(oPar)));
			}
		} */
	...
	public String aObterSituacoesServico() {
		try {
	        ...
	        Set<CpSituacaoConfiguracao> t_setSituacao  = t_cpsServico.getCpTipoServico().getCpSituacoesConfiguracaoSet();
			HashMap<String, String> t_hmpRetorno = new HashMap<String, String>();
			t_hmpRetorno.put("idservico", String.valueOf(t_cpsServico.getIdServico()));
			if (t_setSituacao.size() < 1) {
				throw new AplicacaoException("Serviço sem situação! >> Avise sistemas");
			}
			CpSituacaoConfiguracao t_scfSituacaoDefault = t_cpsServico.getCpTipoServico().getSituacaoDefault();
	        ...
	        for (CpSituacaoConfiguracao t_scfSituacao : t_setSituacao) {
	        	t_hmpRetorno.put("idsituacao_" + t_intContaServico,String.valueOf(t_scfSituacao.getIdSitConfiguracao()));
	        	t_hmpRetorno.put("descsituacao_" + t_intContaServico,t_scfSituacao.getDscSitConfiguracao());
	        	t_hmpRetorno.put("sedefault_" + t_intContaServico,t_scfSituacaoDefault.getIdSitConfiguracao().equals(t_scfSituacao.getIdSitConfiguracao()) ? "true" : "false");
	        	t_intContaServico++;
	        }
	...
	private List<CpServico> obterServicos() {
		return dao().listarTodos(CpServico.class);
	}
	/**
	...
	/**
	 * @param streamRel the streamRel to set
	 */
	...
	/**
	 * @param cpServicos the cpServicos to set
	 */
	...
	/**
	 * @param cpOrgaosUsuario the cpOrgaosUsuario to set
	 */
	...
	/**
	 * @param situacoesSelecionadas the situacoesSelecionadas to set
	 */
	...
	/**
	 * @param idOrgaoUsuario the idOrgaoUsuario to set
	 */
