...
<mod:modelo>
	<mod:entrevista>
		<mod:grupo titulo="Comunicações de Alterações de Frequência">
		...
		<mod:grupo>
			<b> <mod:mensagem titulo=""
				texto="Anotações feitas após a transferência do formulário para a SECAD, não serão consideradas." vermelho="Sim" />
			</b>
		</mod:grupo>
						...
						<c:forEach var="i" begin="1"
							...
							<c:choose>
								...
								<c:when
									...
									<mod:hora titulo="Tempo de Atraso/Saída Antecipada" var="atraso${i}${pes_sigla}" />hs
...
@page {
	...
	margin-bottom: 2cm;
		...
		<b>Comunicações de Alterações de Frequência</b><br />
		<br />
		</p>
								...
								<tr>
									...
									<td bgcolor="#FFFFFF" width="35%" align="center"><c:forEach
										...
										<c:choose>
											...
											<c:when
												test="${requestScope[f:concat(f:concat('motivo',i),pes_sigla)] == 'Atraso/Saída Antecipada'}"> ${requestScope[f:concat(f:concat('data',i),pes_sigla)]} - ${requestScope[f:concat(f:concat('atraso',i),pes_sigla)]} hs - ${requestScope[f:concat(f:concat('motivo',i),pes_sigla)]}
											<c:if
													test="${requestScope[f:concat(f:concat('motivo',i),pes_sigla)] == 'Atraso/Saída Antecipada'}">
