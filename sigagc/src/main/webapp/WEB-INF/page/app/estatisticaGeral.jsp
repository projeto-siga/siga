	...
	<div class="gt-content clearfix">
		...
		<div class="gt-left-col">
			...
			<c:if test="${empty listaMaisRecentes}">
				<h6 style="background:#d8d8c0;padding:3px 10px;">Nenhum conhecimento recente.</h6>
			</c:if> <br />
		...
		<div class="gt-right-col">
				...
				<table class="gt-table gt-user-table">
					...
					<tbody>
						...
						<tr>
							...
							<td style="width: 20%;text-align: right;"><a href="${linkTo[AppController].listar}?filtro.lotacao.id=${p[3]}&filtro.autor.id=${p[1]}&filtro.situacao.idMarcador=36&filtro.pesquisa=true">${p[4]}</a></td>
						</tr>
						</c:forEach>
					</tbody>
				</table>
			...
			<c:if test="${empty listaPrincipaisAutores}">
				<h6 style="background:#d8d8c0;padding:3px 10px;">Nenhum autor.</h6>
			</c:if>
			<br />
			<h2>Principais Lotações</h2>
			<c:if test="${not empty listaPrincipaisLotacoes}">
			<div class="gt-content-box gt-for-table">
				<table class="gt-table gt-user-table">
					<thead>
						<tr>
					...
					<tbody>
						<c:forEach items="${listaPrincipaisLotacoes}" var="l">
						<tr>
			...
			</c:if> <c:if test="${empty listaPrincipaisLotacoes}">
			<h6 style="background:#d8d8c0;padding:3px 10px;">Nenhuma Lotação.</h6>
			</c:if>
			<br />
			<h2>Evolução</h2>
			...
			<p id="dicaGrafico" style="font-size:88.5%;text-align: center;color:#365b6d;color:#365b6d;background:#d8d8c0;padding:3px 10px;">Clique nas barras 
				<span style="color:#B00000;"> vermelhas</span> para saber quais conhecimentos fazem parte dessa 
				estatística no mês selecionado</p>
		</div>
	</div>
</div>
