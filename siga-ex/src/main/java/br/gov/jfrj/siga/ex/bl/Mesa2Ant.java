...
import br.gov.jfrj.siga.ex.bl.Mesa2.DocDados;
import br.gov.jfrj.siga.ex.bl.Mesa2.GrupoItem;
import br.gov.jfrj.siga.ex.bl.Mesa2.Marca;
import br.gov.jfrj.siga.ex.bl.Mesa2.MeM;
import br.gov.jfrj.siga.ex.bl.Mesa2.MesaItem;
import br.gov.jfrj.siga.ex.model.enm.ExTipoDeMovimentacao;
import br.gov.jfrj.siga.hibernate.ExDao;

public class Mesa2Ant {
	...
	public static class GrupoItem implements ISwaggerModel {
		...
		public Long grupoQtd; 			// Qtd possivel de rolagem na tela
		public Long grupoQtdPag;
			...
			for (MeM tag : references.get(mobil).listMeM) {
				...
				if(tag.marca.getCpMarcador().isDemandaJudicial()) {
					t.nome += " até " + tag.marca.getExMobil().getDoc().getMobilGeral()
							...
							.filter(mov -> mov.getExTipoMovimentacao() ==
	...
	public static List<Mesa2Ant.GrupoItem> getContadores(ExDao dao, DpPessoa titular, DpLotacao lotaTitular, 
			Map<String, Mesa2Ant.SelGrupo> selGrupos, boolean exibeLotacao, 
	...
	public static List<Mesa2Ant.GrupoItem> getMesa(ExDao dao, DpPessoa titular,
			DpLotacao lotaTitular, Map<String, Mesa2Ant.SelGrupo> selGrupos, List<Mesa2Ant.GrupoItem> gruposMesa, 
...
//		long tempoIni = System.nanoTime();
		...
		if (l != null && l.size() > 0) {
			...
			for (Mesa2Ant.GrupoItem gItem : gruposMesa) {
						...
						for (Integer i2 = 0; i2 < l.size(); i2++) {  
							...
							if (mobil.getIdMobil() == idMob) {
								...
								if (!map.containsKey(mobil)) {
									...
									if ((gItem.grupoQtd != null && listIdMobil.size() < gItem.grupoQtd) 
				...
				if (!map.isEmpty()) {
					...
					gItem.grupoDocs = Mesa2Ant.listarReferencias(TipoDePainelEnum.UNIDADE, map, titular,
...
//					grpItem.grupoQtdPag = 15L;
	...
	public static List<GrupoItem> montaGruposUsuario(Map<String, Mesa2Ant.SelGrupo> selGrupos) {
		carregaGruposBase();
		...
		if (selGrupos != null && selGrupos.size() > 0) {
			for (Map.Entry<String, Mesa2Ant.SelGrupo> selGrupo : selGrupos.entrySet())
