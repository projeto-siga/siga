	...
	<div class="container-fluid content" id="page">
		...
		<div class="row mt-2">
			<div class="col col-sm-12 col-md-8">
				...
				<h2 class="mt-3">Procedimento ${piId}</h2>
				<siga:links>
							...
							<div class="gt-form-row gt-width-100">
								...
								<c:forEach var="variable" items="${task.variableList}">
									<div class="form-group">
										<c:choose>
											<c:when
												test="${fn:startsWith(variable.identificador,'sel_')}">
										...
										<c:set var="editable"
											value="${variable.access eq 'READ_WRITE'}" />
										<c:if test="${editable}">
										...
										<c:choose>
											<c:when
												test="${fn:startsWith(variable.identificador,'doc_')}">
												<c:choose>
											...
											<c:when
												test="${fn:startsWith(variable.identificador,'pes_')}">
												<c:choose>
													<c:when test="${editable}">
														<siga:selecao propriedade="${variable.identificador}"
									...
									${variable.valor}
											...
											<c:when
												test="${fn:startsWith(variable.identificador,'lot_')}">
												<c:choose>
													<c:when test="${editable}">
														<siga:selecao propriedade="${variable.identificador}"
							...
							<div class="gt-form-row gt-width-100">
								<c:forEach var="transition" items="${task.transitions}">
									<button type="submit" name="transitionName"
										...
										class="btn btn-primary mr-3">${empty transition.name ? 'Prosseguir' : transition.name}${transition.resp}</button>
								</c:forEach>
							</div>
							<c:if test="${empty task.transitions}">
								<button type="submit" name="transitionName" value=""
									class="btn btn-primary mr-3">Prosseguir${transition.resp}</button>
							</c:if>
							<%--</c:if> --%>
							<c:forEach var="variable" items="${task.variableList}">
