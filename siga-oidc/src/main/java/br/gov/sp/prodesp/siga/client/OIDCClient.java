...
public class OIDCClient {
	...
	public OIDCClient() {
		...
		/*
		 * Veriica se já possui conexao
		 * 
		 * if (authRequest == null) { return; } String uriString =
		 * authRequest.toURI().toString();
		 * Loggers.MAIN.debug("Updating OpenID authentication request: " +
		 * uriString); popupOpener.setUrl(uriString);
		 */

		oidcParameter = new LoadProperties();
	}
	...
	private void createProviderDetailsForm() {

		URL issURL;
		try {
			issURL = new URL(oidcParameter.getParametersOIDC().getIss());
		...
		try {
			...
			log.error("Couldn't retrieve OP metadata for {}: {}" + issURL + e.getMessage());
			return;
		} catch (ParseException e) {
			log.info("Invalid OP metadata for {}: {}" + issURL + e.getMessage());
			return;
		}

		log.info("Retrieved OP metadata: {}" + opMetadata.toJSONObject());
	...
	public AuthenticationRequest buildAuthenticationRequest() {
		...
		try {
			...
			pendingRequests.put(authRequest.getState(),
					new PendingAuthenticationRequest(opMetadata, clientInfo, authRequest, codeVerifier));
