...
@NamedQueries({
		...
		@NamedQuery(name = "consultarPorFiltroDpCargoInclusiveInativos", query = "from DpCargo "
				+ "  where idCargo in (select max(cargo.idCargo) from DpCargo cargo "
				+ "     where upper(cargo.nomeCargoAI) like upper('%' || :nome || '%')"
				+ "  	and (:idOrgaoUsu = null or :idOrgaoUsu = 0L or cargo.orgaoUsuario.idOrgaoUsu = :idOrgaoUsu)"
				+ "		group by cargo.idCargoIni)"
		...
		@NamedQuery(name = "consultarQuantidadeDpCargoInclusiveInativos", query = "select count(o) from DpCargo o "
				+ "  where idCargo in (select max(cargo.idCargo) from DpCargo cargo "
				+ "  where upper(cargo.nomeCargoAI) like upper('%' || :nome || '%')"
