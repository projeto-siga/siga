...
public class CpDao extends ModeloDao {
	...
	public List<CpOrgaoUsuario> consultarPorFiltroComContrato(final CpOrgaoUsuarioDaoFiltro o, final int offset,
			final int itemPagina) {
		try {
			Query query = HibernateUtil.getSessao()
	...
	public List<DpPessoa> consultarPorFiltroSemIdentidade(final DpPessoaDaoFiltro flt, final int offset,
		...
		try {
			final Query query;
			boolean isFiltrarPorListaDeUsuario = (flt.getIdPessoaSelecao() != null
					&& flt.getIdPessoaSelecao().length > 0);
			boolean isFiltrarPorListaDeLotacao = (flt.getIdLotacaoSelecao() != null
					&& flt.getIdLotacaoSelecao().length > 0 && !isFiltrarPorListaDeUsuario);
	...
	public List<DpPessoaUsuarioDTO> consultarUsuariosComEnvioDeEmailPendenteFiltrandoPorLotacao(
			final DpPessoaDaoFiltro flt) {
		try {
			final Query query;
			boolean isFiltrarPorListaDeLotacao = (flt.getIdLotacaoSelecao() != null
					&& flt.getIdLotacaoSelecao().length > 0);
	...
	public List<DpPessoa> consultarPessoaComOrgaoFuncaoCargo(final DpPessoa pes) {
		try {
			...
			query = getSessao().getNamedQuery("consultarPessoaComOrgaoFuncaoCargo");

			query.setString("nome", pes.getNomePessoa().toUpperCase().replace(' ', '%'));
	...
	static private Configuration configurarHibernate(Configuration cfg) throws Exception {
		...
		cfg.setCacheConcurrencyStrategy("br.gov.jfrj.siga.dp.DpVisualizacao", "transactional",
				CACHE_QUERY_SUBSTITUICAO);
	...
	public List<CpOrgaoUsuario> consultarOrgaosMarcadosComo(final Long orgaoUsuId, final Long lotacaoId,
		...
		String queryTemp = "select distinct " + "(select orgaoUsu from CpOrgaoUsuario orgaoUsu where "
				+ "		(orgaoUsu1 is null or orgaoUsu.idOrgaoUsu = orgaoUsu1.idOrgaoUsu) "
				+ "		and (orgaoUsu2 is null or orgaoUsu.idOrgaoUsu = orgaoUsu2.idOrgaoUsu)) " + "from ExMarca mar "
				+ "inner join mar.exMobil mob " + "inner join mob.exDocumento doc "
				+ "inner join mar.cpMarcador as marcador " + "left join mar.dpLotacaoIni.orgaoUsuario orgaoUsu1 "
				+ "left join mar.dpPessoaIni.orgaoUsuario orgaoUsu2 "
				+ "where doc.dtDoc >= :dtini and doc.dtDoc < :dtfim " + "		and doc.dtFinalizacao is not null "
				+ queryOrgao + queryLotacao + queryUsuario + "and marcador.idMarcador = :idMarcador "
