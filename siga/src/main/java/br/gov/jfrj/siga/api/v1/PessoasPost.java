	...
	public void run(Request req, Response resp, SigaApiV1Context ctx) throws Exception {
		try {
			...
			if (req.rgUf != null && req.rgUf.length() != 2)
				throw new AplicacaoException("UF do RG deve ser informada com dois caracteres ou deve ser nula.");
			...
			DpPessoa pes = new CpBL().criarUsuario(null, ctx.getIdentidadeCadastrante(), idOrgaoUsu, idCargo, idFuncao,
					idLotacao, nmPessoa, dtNascimento, cpf, email, identidade, orgaoIdentidade, ufIdentidade,
					dataExpedicaoIdentidade, nomeExibicao, enviarEmail);
