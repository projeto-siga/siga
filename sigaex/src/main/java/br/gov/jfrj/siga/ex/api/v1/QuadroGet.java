	...
	public void run(Request req, Response resp, ExApiV1Context ctx) throws Exception {
		...
		if (req.estilo != null && "AGRUPADOS".equals(req.estilo.toUpperCase())) {
			items.sort(new Comparator<QuadroItemVO>() {
				...
				public int compare(QuadroItemVO o1, QuadroItemVO o2) {
					int i = 0;
					i = br.gov.jfrj.siga.base.util.Utils.comparar(o2.finalidadeId, o1.finalidadeId);
					if (i != 0)
						return i;
		...
		for (QuadroItemVO i : items) {
			...
			if (finalidade != null) {
				...
				r.tipoId = finalidade.getIdTpMarcador().name();
				r.tipoNome = finalidade.getIdTpMarcador().getDescricao();
			}
