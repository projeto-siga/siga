...
public class PrincipalAction extends SigaActionSupport {
	...
	public String aSelecionar() throws Exception {
		try {
			...
			//dessa forma a regex verifica se a sigla comeÁa com SR ou sr e termina com n˙meros
			//necess·rio para n„o dar conflito caso exista uma lotaÁ„o que inicie com SR
...
//			if (copiaSigla.matches("^[SR|sr].*[0-9]+$")) {
 			...
 			//alterado formato da sigla de requisi√ß√µes, miss√µes e servi√ßos
			...
			if (response.length == 1 && Integer.valueOf(response[0]) == 0) {
				...
				//ao org„o (sigla_orgao_usu = RJ ou acronimo_orgao_usu = JFRJ) e n„o achar resultado com as opÁıes anteriores 
