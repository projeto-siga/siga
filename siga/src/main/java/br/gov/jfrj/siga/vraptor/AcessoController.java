...
public class AcessoController extends GiControllerSupport {
	...
	@Get("/app/configuracao-acesso")
	public void acessoconf() throws Exception {
		listar();
		
		result.include("itemHTML", itemHTML);
		result.include("itensHTML", itensHTML);
		result.include("pessoaSel", pessoaSel);
		result.include("lotacaoSel", lotacaoSel);
		result.include("perfilSel", perfilSel);
		
		result.include("idServico", idServico);
		result.include("idSituacao", idSituacao);
		result.include("idAbrangencia", idAbrangencia);
		result.include("nomeOrgaoUsu", idOrgaoUsu);		
		result.include("nomeOrgaoUsu", nomeOrgaoUsu);
	}
