...
public class Stamp {
    ...
    public static byte[] stamp(byte[] abPdf, String sigla, boolean rascunho, boolean copia, boolean cancelado,
        ...
        if ((width >= 1189 && width < 2360) && (height >= 841 && height < 2183)) { // aqui é para tratar se a folha que é recebida será um A0 ou não, e para isso peguei altura e largura para tal
        ...
        else if(width >= 2360 && height >= 1085) {
            return stamp(abPdf, sigla, rascunho, copia, cancelado,
                    semEfeito, internoProduzido, qrCode, mensagem, paginaInicial,
                    paginaFinal, cOmitirNumeracao, instancia, orgaoUsu, marcaDaguaDoModelo,
                    idsAssinantes, PageSize.A3);
        }
    ...
    public static byte[] stamp(byte[] abPdf, String sigla, boolean rascunho, boolean copia, boolean cancelado,
        ...
        try (ByteArrayOutputStream bo2 = new ByteArrayOutputStream()) {
            ...
            if (size == PageSize.A0) {
                ...
                while (i < n) {
                    ...
                    image39.setAbsolutePosition(
                            ...
                            120 + BARCODE_HEIGHT_IN_CM * CM_UNIT); // botei 120 para que o código de barras não esconda as informações
                    ...
                    mask.setAbsolutePosition(r.getWidth() - image39.getHeight() - (PAGE_BORDER_IN_CM) * CM_UNIT, 120 + 
                            (BARCODE_HEIGHT_IN_CM - STAMP_BORDER_IN_CM) * CM_UNIT); // botei 120 para que o código de barras não  esconda as informações
                    ...
                    if (stream != null) {
                        ...
                        logo.setAbsolutePosition(
                                r.getWidth() - image39.getHeight() + (STAMP_BORDER_IN_CM - PAGE_BORDER_IN_CM) * CM_UNIT,
                                120 + PAGE_BORDER_IN_CM * CM_UNIT); // botei 120 para que o código de barras não  esconda as informações
                        ...
                        mask.setAbsolutePosition(r.getWidth() - image39.getHeight() - (PAGE_BORDER_IN_CM) * CM_UNIT,
                                120 + (PAGE_BORDER_IN_CM - STAMP_BORDER_IN_CM) * CM_UNIT); // botei 120 para que o código de barras não  esconda as informações
                        mask.scaleAbsolute(image39.getHeight() + 2 * STAMP_BORDER_IN_CM * CM_UNIT,
                    ...
                    if (qrCode != null) {
                        ...
                        imageQRCode.setAbsolutePosition(QRCODE_LEFT_MARGIN_IN_CM * CM_UNIT,
                    ...
                    if (mensagem != null) {
                        ...
                        table.setTotalWidth(r.getWidth() - image39.getHeight() - (QRCODE_LEFT_MARGIN_IN_CM
                                + QRCODE_SIZE_IN_CM + 1000 * STAMP_BORDER_IN_CM + PAGE_BORDER_IN_CM) * CM_UNIT); 
                        PdfPCell cell = new PdfPCell(new Paragraph(mensagem, // tive que colocar o valor de 1000 para não pegar as bordas da tela direita e não sumir as informações
                ...
                while (i < n) {
                    ...
                    if (qrCode != null) {
                        ...
                        Image imageQRCode = Image.getInstance(imgQRCode, Color.BLACK, true);
                        imageQRCode.scaleAbsolute(QRCODE_SIZE_IN_CM * CM_UNIT, QRCODE_SIZE_IN_CM * CM_UNIT);
                        imageQRCode.setAbsolutePosition(QRCODE_LEFT_MARGIN_IN_CM * CM_UNIT + 60, //para afastar o QR Code da borda afim de não sobrepor as informações coloquei 60
                                PAGE_BORDER_IN_CM * CM_UNIT);

                        over.setRGBColorFill(255, 255, 255);
                        mask.setAbsolutePosition((QRCODE_LEFT_MARGIN_IN_CM - STAMP_BORDER_IN_CM) * CM_UNIT + 60, //para afastar a máscara do texto e também do QR Code tive que adicionar 120 
                                (PAGE_BORDER_IN_CM - STAMP_BORDER_IN_CM) * CM_UNIT);
                        mask.scaleAbsolute((QRCODE_SIZE_IN_CM + 2 * STAMP_BORDER_IN_CM) * CM_UNIT,
                                (QRCODE_SIZE_IN_CM + 2 * STAMP_BORDER_IN_CM) * CM_UNIT);
                        over.addImage(mask);
                    ...
                    if (mensagem != null) {
                        ...
                        over.setRGBColorFill(255, 255, 255);
                        mask.setAbsolutePosition(
                                (QRCODE_LEFT_MARGIN_IN_CM + QRCODE_SIZE_IN_CM + STAMP_BORDER_IN_CM) * CM_UNIT + 60, //para afastar a máscara do texto e também do QR Code tive que adicionar 120 
                                (PAGE_BORDER_IN_CM - STAMP_BORDER_IN_CM) * CM_UNIT);
                        mask.scaleAbsolute(2 * STAMP_BORDER_IN_CM * CM_UNIT + table.getTotalWidth(),
                                2 * STAMP_BORDER_IN_CM * CM_UNIT + table.getTotalHeight());
                        over.addImage(mask);
                        ...
                        over.setRGBColorFill(0, 0, 0);
                        table.writeSelectedRows(0, -1,
                                (QRCODE_LEFT_MARGIN_IN_CM + QRCODE_SIZE_IN_CM + 2 * STAMP_BORDER_IN_CM) * CM_UNIT + 60, //para afastar o texto do QR Code tive que adicionar 120 
                                table.getTotalHeight() + PAGE_BORDER_IN_CM * CM_UNIT, over);
                    }
            ...
            else { // se a página não for A0 e nem A3, então trata como A4 = PageSize.A4
                QRCODE_LEFT_MARGIN_IN_CM = 0.6f;
                BARCODE_HEIGHT_IN_CM = 2.0f;
                PAGE_BORDER_IN_CM = 0.5f;
                STAMP_BORDER_IN_CM = 0.2f;
