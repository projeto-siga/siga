...
<!-- 	    <mod:grupo titulo="Período">
		    ...
		    <mod:grupo titulo="Dados do Curso"><br>
		        <mod:texto titulo="Curso" var="curs" largura="40"/>
		        <mod:texto titulo="Cidade" var="cidad" largura="20"/><br>
		        <br>
                <mod:selecao titulo="Acerto Gramatical" var="acgra"  opcoes="no;na;nos;nas" reler="sim" />
		        <mod:texto titulo="País" var="pais" largura="30"/>
		        <mod:texto titulo="Número de Dias" var="nume" largura="5"/><br><br>
		         </mod:grupo>
		         <mod:grupo>
	...
	<mod:documento>
	<c:set var="juiz_federal" value ="${f:pessoa(requestScope['partic_pessoaSel.id'])}" />
	    <mod:valor var="texto_ato">
			...
			<b>AUTORIZAR</b> o afastamento <c:choose><c:when test="${juiz_federal.sexo == 'M'}">do Juiz Federal </c:when><c:otherwise>da Juíza Federal </c:otherwise></c:choose> 
			${juiz_federal.nomePessoa}, para participar do ${curs} em ${cidad}, ${acgra} ${pais},
			<c:choose><c:when test="${nume == '1'}">no dia ${dtin}, </c:when><c:otherwise>por ${nume} (${diasext}) dias, no período de ${dtin} a ${f:calculaData(nume,requestScope['dtin'])},</c:otherwise></c:choose> 
            com ônus limitado aos vencimentos. 
		</mod:valor>  	
		</mod:documento>
</mod:modelo>