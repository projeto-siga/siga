...
public class SrConfiguracaoBL extends CpConfiguracaoBL {
    ...
    public boolean atendeExigencias(CpConfiguracao cfgFiltro, Set<Integer> atributosDesconsiderados, CpConfiguracao cfg, SortedSet<CpPerfil> perfis) {
        ...
        if (cfg instanceof SrConfiguracao && cfgFiltro instanceof SrConfiguracao) {
            SrConfiguracao conf = (SrConfiguracao) cfg;
            SrConfiguracao filtro = (SrConfiguracao) cfgFiltro;

            if (!atributosDesconsiderados.contains(ACAO) && conf.getAcoesSet() != null && conf.getAcoesSet().size() > 0) {
            ...
            if (!atributosDesconsiderados.contains(ITEM_CONFIGURACAO) && conf.getItemConfiguracaoSet() != null && conf.getItemConfiguracaoSet().size() > 0) {
            ...
            if (!atributosDesconsiderados.contains(LISTA_PRIORIDADE) && conf.getListaPrioridade() != null
            ...
            if (!atributosDesconsiderados.contains(TIPO_ATRIBUTO) && conf.getAtributo() != null
                    && (filtro.getAtributo() == null || (filtro.getAtributo() != null && !conf.getAtributo().getAtual().equivale(filtro.getAtributo()))))
                return false;

            if (!atributosDesconsiderados.contains(ATENDENTE) && conf.getAtendente() != null
                    && (filtro.getAtendente() == null || (filtro.getAtendente() != null && !conf.getAtendente().getLotacaoAtual().equivale(filtro.getAtendente()))))
                return false;
    ...
    public List<SrConfiguracao> listarConfiguracoesAtivasPorFiltro(SrConfiguracao confFiltro, int atributoDesconsideradoFiltro[]) throws Exception {
