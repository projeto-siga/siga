...
public class Application extends SigaApplication {
	...
	public static void agendamento_insert(String frm_data_ag,
		...
		try {
			...
			hr = frm_hora_ag;
			// begin transaction, que, segundo o Da Rocha √© autom√°tico; no
			// inicio da action
	...
	public static void agendamento_atualiza(String cod_sala, String data_ag,
		...
		if (objUsuario != null) {
			...
			Locais objSala = Locais.find(
					"cod_forum='" + objUsuario.forumFk.cod_forum
							+ "' and cod_local='" + cod_sala + "'").first(); // isso n„o d· erro no caso de retorno vazio?
	...
	public static void permissao_exclui(String matricula_proibida){
		...
		String lotacaoSessao = cadastrante().getLotacao().getSiglaLotacao();
		UsuarioForum objUsuario = UsuarioForum.find("matricula_usu = '"+matriculaSessao+"'").first();
		if ((objUsuario !=null) && (lotacaoSessao.trim().equals("CSIS"))){ //pode excluir a permiss„o
